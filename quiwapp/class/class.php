...
class Login extends Db
{
	...
	public function ExpiraSession(){
		...
		if( isset($_SESSION['time']) ){ $tiempo = $_SESSION['time']; }
		else{ $tiempo = strtotime(date("Y-m-d h:i:s")); }
...
<?php
	...
	public function RecuperarPassword()
	{
        ...
        //$smtp->Username   = "calatayudjuan17@gmail.com";
        //$smtp->Password   = "jftwbcrabiedjcro";
		//$smtp->Username   = "quiwapp@sess.xyz";
		//$smtp->Password   = "EjBa3_M9_dmq";
		$smtp->Username   = "soporte.quiwapp@gmail.com";
        $smtp->Password   = "0120quiwapp";
		$smtp->Port       = 587;
		//$smtp->Port       = 465;
        $smtp->From       = "soporte.quiwapp@gmail.com"; // from mail
        $smtp->FromName   = "Quiwapp - Restaurantes"; // from mail name
        // Get the form fields and remove whitespace.
        $name = strip_tags(trim($nombres));
        $subject = strip_tags(trim("Quiwapp - Recuperacion Contraseña"));
		...
		$contenidoHTML.="<link rel='stylesheet' href='https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css'>";
        $contenidoHTML.="<title>Recuperacion de Contraseña</title>";
		$contenidoHTML.="</head><body>";
		$contenidoHTML.="<img src='http://www.quiwapp.com/quiwapp/fotos/logo-principal.png' width='200' class='img-fluid rounded'>";
		$contenidoHTML.="<p>Bienvenido Nuevamente $name </p>";
        $contenidoHTML.="<p>Usuario : $name</p>";
        $contenidoHTML.="<p>Nueva Contraseña : $pass</p>";
        $contenidoHTML.="<p>Por favor haga click en el enlace de abajo para iniciar sesion y cambie su contraseña por una nueva</p>";
        $contenidoHTML.="<p>enlace: <a href='https://www.quiwapp.com/quiwapp/index.php'>Iniciar Sesion</a></p>";
        $contenidoHTML.="<p>Medellin-CO: \n\nhttps://www.quiwapp.com/</p>";
        $contenidoHTML.="</body>\n";
        ...
        foreach ($mailTo as $mail=>$name) {
            ...
            if (!$smtp->Send()) {
                ...
                //header("Location: quiwapp/index.php");
...
<?php
	...
	public function ConfiguracionPorIdAdm()
	{
		self::SetNames();
		$sql = " SELECT 
		configuracion.id,
		...
		$stmt = $this->dbh->prepare($sql);
		if($_SESSION["empresa"] != null)
		$stmt->execute(array($_SESSION["empresa"]));
		else if(decrypt($_GET["id"]) != null && $_SESSION["empresa"] == null)
	...
	public function ActualizarConfiguracion(){
		...
		}else{ $fechaautorizacion = limpiar('0000-00-00'); };
		...
		if((strpos($tipo_archivo,'image/png')!==false || (strpos($tipo_archivo,'image/jpeg')!==false))&&$tamano_archivo<200000 ){
			if( move_uploaded_file($_FILES['imagen']['tmp_name'], "fotos/empresa/".$nombre_archivo) 
			&& rename("fotos/empresa/".$nombre_archivo,"fotos/empresa/logo-".$_SESSION["empresa"].".png") ){ 
		...
		if( isset($_FILES['imagen2']['size']) ){
         ...
         //compruebo si las características del archivo son las que deseo  
		if( (strpos($tipo_archivo,'image/png')!==false || (strpos($tipo_archivo,'image/jpeg')!==false))&&$tamano_archivo<200000 ){  
			if (move_uploaded_file($_FILES['imagen2']['tmp_name'], "fotos/empresa/".$nombre_archivo) 
			&& rename("fotos/empresa/".$nombre_archivo,"fotos/empresa/logo-pdf".$_SESSION["empresa"].".png") ){ 
		 		## se puede dar un aviso
			} 
		 	## se puede dar otro aviso 
		}
		################## FINALIZA SUBIR LOGO PDF #1 ######################################
		//header("Location: adm_restaurantes");
		echo "<span class='fa fa-check-square-o'></span> LOS DATOS DE CONFIGURACI&Oacute;N FUERON ACTUALIZADOS EXITOSAMENTE";
	
		exit;
	}

	public function ActualizarConfiguracionAdm(){
		self::SetNames();
		if( empty($_POST["cuit"]) or empty($_POST["nomsucursal"]) or empty($_POST["tlfsucursal"]) ){
			echo "1";
			exit;
		...
		if((strpos($tipo_archivo,'image/png')!==false || (strpos($tipo_archivo,'image/jpeg')!==false))&&$tamano_archivo<200000 ){
			if( move_uploaded_file($_FILES['imagen']['tmp_name'], "fotos/empresa/".$nombre_archivo) 
			&& rename("fotos/empresa/".$nombre_archivo,"fotos/empresa/logo-".$id.".png") ){ 
		 		## se puede dar un aviso
			}
		 	## se puede dar otro aviso 
		}
		...
		if( (strpos($tipo_archivo,'image/png')!==false || (strpos($tipo_archivo,'image/jpeg')!==false))&&$tamano_archivo<200000 ){  
			if (move_uploaded_file($_FILES['imagen2']['tmp_name'], "fotos/empresa/".$nombre_archivo) 
			&& rename("fotos/empresa/".$nombre_archivo,"fotos/empresa/logo-pdf".$id.".png") ){ 
		 		## se puede dar un aviso
			} 
		 	## se puede dar otro aviso 
		}
	...
	public function RegistrarConfiguracion(){  
		...
		if($num == 0){
			...
			echo "<span class='fa fa-check-square-o'></span> SE HA SIDO REGISTRADO EXITOSAMENTE";
			//header("Location: adm_restaurantes");
			exit;
	...
	public function Notificaciones(){
		...
		$sql = "SELECT notificaciones.id_notf, mesas.nommesa FROM notificaciones JOIN mesas ON notificaciones.mesa = mesas.codmesa WHERE notificaciones.estado = 'A' AND notificaciones.empresa = ". $_SESSION["empresa"];;
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
		//return $this->p;
		echo json_encode($this->p);
		$this->dbh=null;
	}

	public function CerrarNotificacion(){
		$sql = "UPDATE notificaciones set estado = ? WHERE id_notf = ?";
		$estado = "B";
					$stmt = $this->dbh->prepare($sql);
					$stmt->bindParam(1, $estado);
					$stmt->bindParam(2, $id);
					$id = limpiar($_POST["id_notf"]);
					$stmt->execute();
	...
	############################## FUNCION LISTAR RESTAURANTES ################################
	############################# FUNCION ELIMINAR CONFIGURACION ################################
	...
	public function RegistrarUsuariosAdm()
	{
		self::SetNames();
		if(empty($_POST["nombres"]) or empty($_POST["usuario"]) or empty($_POST["password"]) or empty($_POST["empresa"]))
		{
			echo "1";
			exit;
		}
	...
	public function RegistrarUsuariosAdmin()
	{
		...
		else
		{
			...
			else
			{
				...
				if($num == 0)
				{
					$query = " INSERT INTO usuarios values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, null);";
					$stmt = $this->dbh->prepare($query);
					$stmt->bindParam(1, $dni);
					$stmt->bindParam(2, $nombres);
					$stmt->bindParam(3, $sexo);
	...
	public function ListarUsuarios()
	{
		...
		if($_SESSION["empresa"] == null){
			...
			$sql = "SELECT * FROM usuarios where empresa=" . $_SESSION["empresa"] . "";
		}
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
	...
	public function ListarUsuariosAll()
	{
		...
		$sql = "SELECT * FROM usuarios JOIN configuracion ON usuarios.empresa = configuracion.id";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
	...
	public function ListarUsuariosADM()
	{
		...
		$sql = "SELECT * FROM usuarios WHERE nivel = 'SUPERADMINISTRADOR(A)'";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
	...
	public function ListarLog(){
		self::SetNames();
		$sql = "SELECT * FROM log WHERE restaurante = ".$_SESSION["empresa"];
		foreach( $this->dbh->query($sql) as $row ){ $this->p[] = $row; }
		return $this->p;
		$this->dbh=null;
   	}
	...
	public function ActualizarUsuarios()
	{
		...
		self::SetNames();
		if(empty($_POST["dni"]) or empty($_POST["nombres"]) or empty($_POST["usuario"]) or empty($_POST["password"]) or empty($_POST["empresa"]))
		{
			echo "1";
			exit;
		}
		...
		else
		{
			...
			################## SUBIR FOTO DE USUARIOS ######################################
	         ...
	         //compruebo si las características del archivo son las que deseo  
					if ((strpos($tipo_archivo,'image/jpeg')!==false) && $tamano_archivo<50000) 
					{  
						if (move_uploaded_file($_FILES['imagen']['tmp_name'], "fotos/perfil/".$nombre_archivo) && rename("fotos/perfil/".$nombre_archivo,"fotos/perfil/".$dni."-".$empresa.".jpg"))
						{ 
			 ## se puede dar un aviso
						} 
			 ## se puede dar otro aviso 
	...
	public function SeleccionaDepartamento(){
		...
		$stmt->execute(array($_GET["id_provincia"]));
		$num = $stmt->rowCount();
		if($num==0){ echo "<option value=''> -- SIN RESULTADOS -- </option>"; exit;	}
		else{ 
			while($row = $stmt->fetch() ){ $this->p[]=$row; }
			return $this->p;
			$this->dbh=null;
		}
	}
...
public function RegistrarImpuestos()
{
	...
	if(empty($_POST["nomimpuesto"]) or empty($_POST["valorimpuesto"]) or empty($_POST["statusimpuesto"]))
	{
		...
		exit;
			...
			$sql = " SELECT statusimpuesto FROM impuestos WHERE nomimpuesto != ? AND statusimpuesto = ? AND empresa = ".$_SESSION["empresa"]." AND statusimpuesto = 'ACTIVO'";
			$stmt = $this->dbh->prepare($sql);
			...
			$sql = " SELECT nomimpuesto FROM impuestos WHERE nomimpuesto = ? AND empresa = ?";
			$stmt = $this->dbh->prepare($sql);
			$stmt->execute(array($_POST["nomimpuesto"], $_SESSION["empresa"]));
			$num = $stmt->rowCount();
			if($num == 0)
			{
...
{
	self::SetNames();
	$sql = "SELECT * FROM impuestos WHERE empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function ImpuestosPorId()
{
	...
	$stmt = $this->dbh->prepare($sql);
	$stmt->execute(array("ACTIVO", $_SESSION["empresa"]));
	$num = $stmt->rowCount();
	if($row = $stmt->fetch(PDO::FETCH_ASSOC))
		{
			$this->p[] = $row;
...
public function ActualizarImpuestos()
{
	...
	if(empty($_POST["codimpuesto"]) or empty($_POST["nomimpuesto"]) or empty($_POST["valorimpuesto"]) or empty($_POST["statusimpuesto"]))
	{
		...
		exit;
			...
			$sql = " SELECT statusimpuesto FROM impuestos WHERE codimpuesto != ? AND statusimpuesto = ? AND statusimpuesto = 'ACTIVO' AND empresa = ".$_SESSION["empresa"];
			$stmt = $this->dbh->prepare($sql);
			...
			$sql = "SELECT nomimpuesto FROM impuestos WHERE codimpuesto != ? AND nomimpuesto = ? AND empresa = ".$_SESSION["empresa"];
			$stmt = $this->dbh->prepare($sql);
...
public function RegistrarSalas()
{
	...
	if(empty($_POST["nomsala"]))
	{
		...
		exit;
			...
			$sql = " SELECT nomsala FROM salas WHERE empresa = ".$_SESSION["empresa"]." AND nomsala = ?";
			$stmt = $this->dbh->prepare($sql);
			...
			if($num == 0)
			{
				...
				$stmt->bindParam(3, $empresa_sa);

				$nomsala = limpiar($_POST["nomsala"]);
		        $fecha = limpiar(date("Y-m-d"));
		        $empresa_sa = limpiar($_SESSION["empresa"]);
				$stmt->execute();
...
{
	self::SetNames();
	$sql = "SELECT * FROM salas where empresa = " . $_SESSION["empresa"] . "";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function RegistrarMesas()
{
	...
	if(empty($_POST["codsala"]) or empty($_POST["nommesa"]))
	{
		...
		exit;
			...
			$sql = " SELECT codsala, nommesa FROM mesas WHERE codsala = ? and nommesa = ? and empresa = ?";
			$stmt = $this->dbh->prepare($sql);
			$stmt->execute(array($_POST["codsala"], $_POST["nommesa"], $_SESSION["empresa"]));
			$num = $stmt->rowCount();
			if($num == 0)
			{
		        ...
		        $stmt->bindParam(5, $empresa_me);

				$codsala = limpiar($_POST["codsala"]);
				$nommesa = limpiar($_POST["nommesa"]);
		        $fecha = limpiar(date("Y-m-d"));
		        $statusmesa = limpiar("0");
		        $empresa_me = limpiar($_SESSION["empresa"]);
				$stmt->execute();
...
{
	self::SetNames();
	$sql = "SELECT salas.codsala, salas.nomsala, salas.fecha, mesas.codmesa, mesas.nommesa, mesas.fecha, mesas.statusmesa FROM mesas LEFT JOIN salas ON mesas.codsala = salas.codsala 
		where mesas.empresa = " . $_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function RegistrarCategorias()
{
	...
	if(empty($_POST["nomcategoria"]))
	{
		...
		exit;
			...
			$sql = " SELECT nomcategoria FROM categorias WHERE nomcategoria = ? AND empresa = ".$_SESSION["empresa"];
			$stmt = $this->dbh->prepare($sql);
...
{
	self::SetNames();
	$sql = "SELECT * FROM categorias WHERE empresa = ".$_SESSION["empresa"]." ORDER BY nomcategoria ASC";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
############################### FUNCION CARGAR CLIENTES ##############################
	public function CargarClientes()
	{
		...
		$empresa_cl = limpiar($_SESSION["empresa"]);//add empresa
		$stmt->execute();
...
############################ FUNCION LISTAR CLIENTES ################################
	...
	$sql = "SELECT
		...
		where clientes.empresa = " . $_SESSION["empresa"] . "";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
############################ FUNCION REGISTRAR PROVEEDORES ##########################
	public function RegistrarProveedores()
	{
		...
		$sql = " SELECT cuitproveedor FROM proveedores WHERE cuitproveedor = ? AND id_empresa";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array($_POST["cuitproveedor"]),array($_SESSION["empresa"]));
		$num = $stmt->rowCount();
		if($num == 0)
		{
			$query = " INSERT INTO proveedores values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
...
########################### FUNCION LISTAR PROVEEDORES ################################
	public function ListarProveedores()
	{
		...
		WHERE proveedores.id_empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION REGISTRAR INGREDIENTES ###############################
	public function RegistrarIngredientes()
	{
		...
		$sql = " SELECT codingrediente FROM ingredientes WHERE codingrediente = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
...
########################### FUNCION LISTAR INGREDIENTES ################################
	...
	LEFT JOIN proveedores ON ingredientes.codproveedor=proveedores.codproveedor WHERE ingredientes.empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION LISTAR INGREDIENTES EN STOCK MINIMO ################################
	...
	WHERE ingredientes.cantingrediente <= ingredientes.stockminimo AND ingredientes.empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION LISTAR INGREDIENTES EN STOCK MAXIMO ################################
	...
	FROM (ingredientes INNER JOIN medidas ON ingredientes.codmedida=medidas.codmedida)
	LEFT JOIN proveedores ON ingredientes.codproveedor=proveedores.codproveedor
	WHERE ingredientes.cantingrediente >= ingredientes.stockmaximo AND ingredientes.empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function BuscarKardexIngrediente() 
		...
		WHERE kardex_ingredientes.codingrediente = ? AND ingredientes.empresa = ".$_SESSION["empresa"]." AND kardex_ingredientes.empresa = ".$_SESSION["empresa"];
        $stmt = $this->dbh->prepare($sql);
...
public function BuscarIngredientesVendidos() 
       ...
       LEFT JOIN medidas ON ingredientes.codmedida = medidas.codmedida 
       WHERE DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') >= ? AND DATE_FORMAT(ventas.fechaventa,'%Y-%m-%d') <= ?  GROUP BY ingredientes.codingrediente AND ingredientes.empresa = ".$_SESSION["empresa"];
...
############################### FUNCION CARGAR PRODUCTOS ##############################
	public function CargarProductos()
	{
		...
		$sql = " SELECT porcentaje FROM configuracion WHERE id = ".$_SESSION["empresa"];
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
		}
    ...
    $query = "INSERT INTO productos values (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    	...
    	$empresa_pr = limpiar($_SESSION["empresa"]);
    	$stmt->execute();
...
########################### FUNCION REGISTRAR PRODUCTOS ###############################
	public function RegistrarProductos()
	{
		...
		$sql = " SELECT codproducto FROM productos WHERE codproducto = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
...
########################### FUNCION LISTAR PRODUCTOS ################################
	...
	where productos.empresa = " . $_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION LISTAR PRODUCTOS EN STOCK MINIMO ################################
	...
	FROM (productos INNER JOIN categorias ON productos.codcategoria=categorias.codcategoria)
	LEFT JOIN proveedores ON productos.codproveedor=proveedores.codproveedor
	WHERE productos.existencia <= productos.stockminimo and productos.empresa = " . $_SESSION["empresa"] . "";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION LISTAR PRODUCTOS EN STOCK MAXIMO ################################
	...
	FROM (productos INNER JOIN categorias ON productos.codcategoria=categorias.codcategoria)
	LEFT JOIN proveedores ON productos.codproveedor=proveedores.codproveedor
	WHERE productos.existencia >= productos.stockmaximo and productos.empresa = " . $_SESSION["empresa"] . "";
	foreach ($this->dbh->query($sql) as $row) 
	{
		$this->p[] = $row;
	}
...
############################# FUNCION LISTAR PRODUCTOS ################################
	public function ListarProductosModal()
	{
        ...
        INNER JOIN categorias ON productos.codcategoria=categorias.codcategoria WHERE productos.empresa = ".$_SESSION["empresa"];
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
if ((strpos($tipo_archivo,'image/jpeg')!==false)&&$tamano_archivo<200000) 
		 {  
if (move_uploaded_file($_FILES['imagen']['tmp_name'], "fotos/productos/".$nombre_archivo) && rename("fotos/productos/".$nombre_archivo,"fotos/productos/".$codproducto."-".$empresa.".jpg"))
		 { 
		 ## se puede dar un aviso
		 } 
		 ## se puede dar otro aviso 
...
public function BuscarKardexProducto() 
		...
		LEFT JOIN proveedores ON productos.codproveedor=proveedores.codproveedor 
		WHERE kardex_productos.codproducto = ".$_GET["codproducto"]." AND productos.empresa = ".$_SESSION["empresa"]." AND proveedores.id_empresa = ".$_SESSION["empresa"]." AND kardex_productos.empresa = ".$_SESSION["empresa"];
        $stmt = $this->dbh->prepare($sql);
		$stmt->execute();
		$num = $stmt->rowCount();
		if($num==0)
...
############################# FUNCION REGISTRAR COMPRAS #############################
	...
	if(empty($_POST["codcompra"]) or empty($_POST["fechaemision"]) or empty($_POST["fecharecepcion"]) or empty($_POST["codproveedor"]))
        ...
        $sql = "SELECT codcompra FROM compras WHERE codcompra = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
...
if (limpiar($_POST["tipocompra"]=="CONTADO")) { $statuscompra = limpiar("PAGADA"); } else { $statuscompra = "PENDIENTE"; }
		...
		$empresa_co = limpiar($_SESSION["empresa"]);
		$stmt->execute();
...
if (limpiar($detalle[$i]['fechaexpiracion']=="")) { $fechaexpiracionc = "0000-00-00";  } else { $fechaexpiracionc = limpiar(date("Y-m-d",strtotime($detalle[$i]['fechaexpiracion']))); }
	...
	if(limpiar($detalle[$i]['tipoentrada'])=="PRODUCTO"){
		...
		$sql = "SELECT existencia FROM productos WHERE  empresa = " . $_SESSION["empresa"] . " and 
		codproducto = '".limpiar($detalle[$i]['txtCodigo'])."' ";
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
...
public function ListarCompras()
{
	...
	WHERE compras.empresa = " . $_SESSION["empresa"] . " and compras.statuscompra = 'PAGADA' GROUP BY detallecompras.codcompra";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function ListarCuentasxPagar()
{
	...
	LEFT JOIN proveedores ON compras.codproveedor = proveedores.codproveedor 
	LEFT JOIN documentos ON proveedores.documproveedor = documentos.coddocumento
	LEFT JOIN usuarios ON compras.codigo = usuarios.codigo WHERE compras.statuscompra = 'PENDIENTE' AND compras.empresa = ".$_SESSION["empresa"]." AND detallecompras.empresa = ".$_SESSION["empresa"]." GROUP BY detallecompras.codcompra";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
############################## FUNCION ACTUALIZAR COMPRAS #############################
	...
	if(limpiar($_POST['tipoentrada'][$i])=="PRODUCTO"){
			...
			$sql = "SELECT cantingrediente FROM ingredientes WHERE empresa = ".$_SESSION["empresa"]." AND codingrediente = '".limpiar($_POST['codproducto'][$i])."'";
			foreach ($this->dbh->query($sql) as $row)
			{
				$this->p[] = $row;
			}
...
###################### FUNCION BUSQUEDA COMPRAS POR FECHAS ###########################
	public function BuscarComprasxFechas() 
	{
		...
		LEFT JOIN provincias ON proveedores.id_provincia = provincias.id_provincia 
		LEFT JOIN departamentos ON proveedores.id_departamento = departamentos.id_departamento
		 WHERE DATE_FORMAT(compras.fecharecepcion,'%Y-%m-%d') >= ? AND DATE_FORMAT(compras.fecharecepcion,'%Y-%m-%d') <= ? AND compras.empresa = ? GROUP BY detallecompras.codcompra";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindValue(1, trim(date("Y-m-d",strtotime($_GET['desde']))));
		$stmt->bindValue(2, trim(date("Y-m-d",strtotime($_GET['hasta']))));
...
public function RegistrarCajas()
{
	...
	if(empty($_POST["nrocaja"]) or empty($_POST["nomcaja"]) or empty($_POST["codigo"]))
	{
		...
		$sql = "SELECT nrocaja FROM cajas WHERE nrocaja = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
		...
		$sql = "SELECT nomcaja FROM cajas WHERE nomcaja = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
		...
		if($num == 0)
		{
			...
			$stmt->bindParam(3, $codigo);
			$stmt->bindParam(4, $empresa_c);

			$nrocaja = limpiar($_POST["nrocaja"]);
			$nomcaja = limpiar($_POST["nomcaja"]);
			$codigo = limpiar($_POST["codigo"]);
			$empresa_c = limpiar($_SESSION["empresa"]);//add empresa
			$stmt->execute();
...
public function ListarCajas()
{
	...
	if($_SESSION['acceso'] == "administrador" || $_SESSION['acceso'] == "superadministrador" ) {
        ...
        $sql = "SELECT * FROM cajas INNER JOIN usuarios ON cajas.codigo = usuarios.codigo where cajas.empresa=".
        $_SESSION["empresa"] . "";
			foreach ($this->dbh->query($sql) as $row)
			{
				$this->p[] = $row;
...
public function ListarCajasAbiertas()
{
	...
	$sql = "SELECT * FROM cajas INNER JOIN arqueocaja ON cajas.codcaja = arqueocaja.codcaja LEFT JOIN usuarios ON cajas.codigo = usuarios.codigo WHERE arqueocaja.statusarqueo = '1' AND  cajas.empresa = ".$_SESSION["empresa"];
		foreach ($this->dbh->query($sql) as $row)
	    {
		$this->p[] = $row;
	    }
        ...
        $sql = "SELECT * FROM cajas INNER JOIN arqueocaja ON cajas.codcaja = arqueocaja.codcaja LEFT JOIN usuarios ON cajas.codigo = usuarios.codigo WHERE cajas.empresa = ".$_SESSION["empresa"]." AND cajas.codigo = '".limpiar($_SESSION["codigo"])."'";
			foreach ($this->dbh->query($sql) as $row)
	    {
		$this->p[] = $row;
	    }
...
public function ActualizarCajas()
{
	...
	if(empty($_POST["codcaja"]) or empty($_POST["nrocaja"]) or empty($_POST["nomcaja"]) or empty($_POST["codigo"]))
	{
		...
		$sql = "SELECT nomcaja FROM cajas WHERE codcaja != ? AND nomcaja = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
		...
		$sql = "SELECT codigo FROM cajas WHERE codcaja != ? AND codigo = ? AND empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
...
public function ListarArqueoCaja()
{
	...
	if($_SESSION['acceso'] == "administrador" || $_SESSION['acceso'] == "superadministrador" ) {
        ...
        $sql = "SELECT * FROM arqueocaja INNER JOIN cajas ON arqueocaja.codcaja = cajas.codcaja LEFT JOIN usuarios ON cajas.codigo = usuarios.codigo WHERE cajas.empresa = ".$_SESSION["empresa"];
			foreach ($this->dbh->query($sql) as $row)
			{
				$this->p[] = $row;
			}
        ...
        $sql = "SELECT * FROM arqueocaja INNER JOIN cajas ON arqueocaja.codcaja = cajas.codcaja LEFT JOIN usuarios ON cajas.codigo = usuarios.codigo WHERE cajas.codigo = '".limpiar($_SESSION["codigo"])."' AND cajas.empresa = usuarios.empresa";
			foreach ($this->dbh->query($sql) as $row)
			{
				$this->p[] = $row;
			}
...
$sql = "SELECT clientes.dnicliente, clientes.nomcliente, ventas.codpedido, ventas.codcaja, ventas.codcliente, documentos.documento, ventas.subtotalivasi, ventas.subtotalivano, ventas.iva, ventas.totaliva, ventas.descuento, ventas.totaldescuento, ventas.totalpago, ventas.totalpago2, ventas.codigo, ventas.observaciones, detallepedidos.coddetallepedido, detallepedidos.codpedido, detallepedidos.pedido, detallepedidos.codproducto, detallepedidos.producto, detallepedidos.cantventa, detallepedidos.ivaproducto, detallepedidos.descproducto, detallepedidos.valortotal, detallepedidos.totaldescuentov, detallepedidos.valorneto, salas.nomsala, mesas.codmesa, mesas.nommesa, usuarios.nombres 
    ...
    WHERE mesas.codmesa = ? and mesas.statusmesa = 1 AND ventas.statuspago = 1 AND ventas.empresa = ".$_SESSION["empresa"];
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array(decrypt($_GET["codmesa"])));
		$num = $stmt->rowCount();
		if($num==0)
...
<div class="row">
    <div class="col-md-10">
        <div class="form-group has-feedback">
            ...
            <input type="hidden" name="codcliente" id="codcliente" <?php if (isset($reg[0]['codventa'])) { ?>
                value="<?php echo $reg[0]['codcliente'] == '' ? "0" : $reg[0]['codcliente']; ?>" <?php } else { ?>
                value="0" <?php } ?>>
            <input type="text" class="form-control" name="busqueda" id="busqueda"
                onKeyUp="this.value=this.value.toUpperCase();"
                placeholder="Ingrese Criterio para la Búsqueda del Cliente" <?php if (isset($reg[0]['codventa'])) { ?>
                value="<?php echo $reg[0]['codcliente'] == '' ? "CONSUMIDOR FINAL" : $reg[0]['documento3'].": ".$reg[0]['dnicliente'].": ".$reg[0]['nomcliente']; ?>"
...
<div class="pull-left">
    <input id="boton" onClick="mostrar();" style="cursor: pointer;" class="btn btn-info waves-effect waves-light"
        value="Ver Mesas" type="button">
    <div id="remision" style="display: none;"></div>
</div>

<div class="text-right">
    <button type="submit" name="btn-nuevo" id="btn-nuevo" class="btn btn-warning"><span class="fa fa-save"></span>
        Guardar</button>
...
############################ FUNCION REGISTRAR VENTAS ##############################
	public function NuevoPedido()
	{
		...
		$sql = "SELECT codpedido FROM ventas WHERE empresa = ".$_SESSION["empresa"]." ORDER BY idventa DESC LIMIT 1";
		foreach ($this->dbh->query($sql) as $row){
...
if ($_SESSION['acceso'] == "administrador" || $_SESSION['acceso'] == "superadministrador" ) {
	...
	LEFT JOIN usuarios ON ventas.codigo = usuarios.codigo WHERE ventas.statuspago = '0' AND ventas.empresa = ".$_SESSION["empresa"]." GROUP BY detalleventas.codventa";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
public function ListarCreditos()
{
	...
	LEFT JOIN documentos ON clientes.documcliente = documentos.coddocumento
	WHERE ventas.tipopago ='CREDITO' AND ventas.empresa = ".$_SESSION["empresa"]." GROUP BY ventas.codventa";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
 ...
 public function SumaCompras()
{
	...
	FROM compras 
	WHERE YEAR(fecharecepcion) = '".date('Y')."' AND MONTH(fecharecepcion) AND compras.empresa = ".$_SESSION["empresa"]." GROUP BY MONTH(fecharecepcion) ORDER BY 1";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
 ...
 public function SumaVentas()
{
	...
	FROM ventas 
	WHERE YEAR(fechaventa) = '".date('Y')."' AND MONTH(fechaventa) AND ventas.empresa = ".$_SESSION["empresa"]." GROUP BY MONTH(fechaventa) ORDER BY 1";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################### FUNCION PRODUCTOS 5 MAS VENDIDOS ############################
	...
	$sql = "SELECT productos.codproducto, productos.producto, productos.codcategoria, detalleventas.descproducto, detalleventas.precioventa, productos.existencia, categorias.nomcategoria, ventas.fechaventa, SUM(detalleventas.cantventa) as cantidad FROM (ventas LEFT JOIN detalleventas ON ventas.codventa=detalleventas.codventa) LEFT JOIN productos ON detalleventas.codproducto=productos.codproducto LEFT JOIN categorias ON categorias.codcategoria=productos.codcategoria WHERE productos.empresa = ".$_SESSION["empresa"]." AND ventas.empresa = ".$_SESSION["empresa"]." GROUP BY detalleventas.codproducto, detalleventas.precioventa, detalleventas.descproducto ORDER BY productos.codproducto ASC LIMIT 8";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
########################## FUNCION SUMAR VENTAS POR USUARIOS ##########################
	...
	{
		self::SetNames();
     $sql = "SELECT usuarios.codigo, usuarios.nombres, SUM(ventas.totalpago) as total FROM (usuarios INNER JOIN ventas ON usuarios.codigo=ventas.codigo) WHERE usuarios.empresa = ".$_SESSION["empresa"]." GROUP BY usuarios.codigo";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
...
$sql = "SELECT
(SELECT COUNT(codigo) FROM usuarios) AS usuarios,
(SELECT COUNT(codproducto) FROM productos WHERE productos.empresa = ".$_SESSION["empresa"].") AS productos,
(SELECT COUNT(codingrediente) FROM ingredientes WHERE ingredientes.empresa = ".$_SESSION["empresa"].") AS ingredientes,
(SELECT COUNT(codcliente) FROM clientes WHERE clientes.empresa = ".$_SESSION["empresa"].") AS clientes,
(SELECT COUNT(codproveedor) FROM proveedores WHERE proveedores.id_empresa = ".$_SESSION["empresa"].") AS proveedores,
(SELECT COUNT(codproducto) FROM productos WHERE productos.empresa = ".$_SESSION["empresa"]." AND existencia <= stockminimo) AS minimo,
(SELECT COUNT(codproducto) FROM productos WHERE productos.empresa = ".$_SESSION["empresa"]." AND fechaexpiracion != '0000-00-00' AND fechaexpiracion <= '".date("Y-m-d")."') AS vencidos,
(SELECT COUNT(idcompra) FROM compras WHERE empresa = ".$_SESSION["empresa"].") AS compras,
(SELECT COUNT(idventa) FROM  ventas WHERE ventas.empresa = ".$_SESSION["empresa"].") AS ventas,
(SELECT COUNT(idcompra) FROM compras WHERE tipocompra = 'CREDITO' AND statuscompra = 'PENDIENTE' AND fechavencecredito <= '".date("Y-m-d")."') AS creditoscomprasvencidos,
(SELECT COUNT(idventa) FROM ventas WHERE empresa = ".$_SESSION["empresa"]." AND tipopago = 'CREDITO' AND statusventa = 'PENDIENTE' AND fechavencecredito <= '".date("Y-m-d")."') AS creditosventasvencidos";
}
		foreach ($this->dbh->query($sql) as $row)
		{
			$this->p[] = $row;
