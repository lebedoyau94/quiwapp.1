...
class Json
{
    ...
    $consulta = "SELECT CONCAT(nomcategoria) as label, codcategoria FROM categorias WHERE empresa = ".$_SESSION["empresa"]." CONCAT(nomcategoria) LIKE '%".$filtro."%' ORDER BY codcategoria ASC LIMIT 0,10";
    ...
    $consulta = "SELECT CONCAT(productos.producto) as label, productos.codproducto, productos.producto, productos.codcategoria, productos.preciocompra, productos.precioventa, productos.existencia, productos.ivaproducto, productos.descproducto, productos.fechaelaboracion, productos.fechaexpiracion, categorias.nomcategoria FROM productos INNER JOIN categorias ON productos.codcategoria=categorias.codcategoria WHERE productos.empresa = ".$_SESSION["empresa"]." AND CONCAT(codproducto, '',producto, '',nomcategoria, '',codigobarra) LIKE '%".$filtro."%' GROUP BY codproducto ASC LIMIT 0,15";
        $conexion = new conectorDB;
		$this->json = $conexion->EjecutarSentencia($consulta);
		return $this->json;
	}
    ...
    FROM
     ...
     LEFT JOIN
       (SELECT
           ...
           WHERE clientes.empresa = ".$_SESSION["empresa"]." AND CONCAT(clientes.dnicliente, '',clientes.nomcliente) LIKE '%".$filtro."%' 
