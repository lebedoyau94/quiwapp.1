...
$('document').ready(function () {
	...
	$("#saveuseradm").validate({
		...
		messages:
		{
			...
			nivel: { required: "Seleccione Nivel de Acceso" },
			status: { required: "Seleccione Status de Acceso" },
			empresa: { required: "Seleccione Un Restaurante" },
			comision: { required: "Ingrese Comisi&oacute;n por Ventas", number: "Ingrese solo numeros con dos decimales" },
		},
		submitHandler: function (form) {

			var data = $("#saveuseradm").serialize();
			var formData = new FormData($("#saveuseradm")[0]);

			$.ajax({
				type: 'POST',
				url: 'adm_usuarios.php',
				async: false,
				data: formData,
				//necesario para subir archivos via ajax
				cache: false,
...
$('document').ready(function () {
	...
	$("#saveadmin").validate({
		...
		submitHandler: function (form) {
			...
			var data = $("#saveadmin").serialize();
			var formData = new FormData($("#saveadmin")[0]);
				...
				success: function (data) {
					if (data == 1) {
						...
						$("#save").fadeIn(1000, function () {
							...
							$('#usuarios').append('<center><i class="fa fa-spin fa-spinner"></i> Por favor espere, cargando registros ......</center>').fadeIn("slow");
							setTimeout(function () {
								$('#usuarios').load("consultas?CargaUsuariosAdministadores=si");
							}, 2000);
