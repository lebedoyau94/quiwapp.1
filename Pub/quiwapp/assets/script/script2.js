...
function UpdateUsuario(codigo, dni, nombres, sexo, direccion, telefono, email, usuario, nivel, status, comision, empresa, proceso) {
  ...
  $("#saveuser #empresa").val(empresa);
  $("#saveuser #comision").val(comision);
  $("#saveuser #proceso").val(proceso);
}

function UpdateUsuarioAdm(codigo, dni, nombres, sexo, direccion, telefono, email, usuario, nivel, status, comision, empresa, proceso) {
  // aqui asigno cada valor a los campos correspondientes
  $("#saveuseradm #codigo").val(codigo);
  $("#saveuseradm #dni").val(dni);
  $("#saveuseradm #nombres").val(nombres);
  $("#saveuseradm #sexo").val(sexo);
  $("#saveuseradm #direccion").val(direccion);
  $("#saveuseradm #telefono").val(telefono);
  $("#saveuseradm #email").val(email);
  $("#saveuseradm #usuario").val(usuario);
  $("#saveuseradm #nivel").val(nivel);
  $("#saveuseradm #status").val(status);
  $("#saveuseradm #empresa").val(empresa);
  $("#saveuseradm #comision").val(comision);
  $("#saveuseradm #proceso").val(proceso);
}

function UpdateUsuarioAdmin(codigo, dni, nombres, sexo, direccion, telefono, email, usuario, nivel, status, proceso) {
  // aqui asigno cada valor a los campos correspondientes
  $("#saveadmin #codigo").val(codigo);
  $("#saveadmin #dni").val(dni);
  $("#saveadmin #nombres").val(nombres);
  $("#saveadmin #sexo").val(sexo);
  $("#saveadmin #direccion").val(direccion);
  $("#saveadmin #telefono").val(telefono);
  $("#saveadmin #email").val(email);
  $("#saveadmin #usuario").val(usuario);
  $("#saveadmin #nivel").val(nivel);
  $("#saveadmin #status").val(status);
  $("#saveadmin #proceso").val(proceso);
}
...
function VerEmpresa(id) {
  ...
  $('#muestrarestaurantemodal').html('<center><i class="fa fa-spin fa-spinner"></i> Cargando información, por favor espere....</center>');

  var dataString = 'BuscaEmpresaModal=si&id=' + id;
  ...
  $.ajax({
    ...
    data: dataString,
    success: function (response) {
      $('#muestrarestaurantemodal').empty();
      $('#muestrarestaurantemodal').append('' + response + '').fadeIn("slow");
...
function UpdateEmpresa(id, documsucursal, cuit, nomsucursal, tlfsucursal, correosucursal, id_provincia,
  id_departamento, direcsucursal, documencargado, dniencargado, nomencargado, tlfencargado, descuentoglobal,
  porcentaje, codmoneda, codmoneda2, nroactividad, iniciofactura, fechaautorizacion, llevacontabilidad, proceso) {
  // aqui asigno cada valor a los campos correspondientes
  $("#savempresa #id").val(id);
  $("#savempresa #documsucursal").val(documsucursal);
  $("#savempresa #cuit").val(cuit);
  $("#savempresa #nomsucursal").val(nomsucursal);
  $("#savempresa #tlfsucursal").val(tlfsucursal);
  $("#savempresa #correosucursal").val(correosucursal);
  $("#savempresa #id_provincia").val(id_provincia);
  $("#savempresa #id_departamento").val(id_departamento);
  $("#savempresa #direcsucursal").val(direcsucursal);
  $("#savempresa #documencargado").val(documencargado);
  $("#savempresa #dniencargado").val(dniencargado);
  $("#savempresa #nomencargado").val(nomencargado);
  $("#savempresa #tlfencargado").val(tlfencargado);
  $("#savempresa #descuentoglobal").val(descuentoglobal);
  $("#savempresa #porcentaje").val(porcentaje);
  $("#savempresa #codmoneda").val(codmoneda);
  $("#savempresa #codmoneda2").val(codmoneda2);
  $("#savempresa #nroactividad").val(nroactividad);
  $("#savempresa #iniciofactura").val(iniciofactura);
  $("#savempresa #fechaautorizacion").val(fechaautorizacion);
  $("#savempresa #llevacontabilidad").val(llevacontabilidad);
  $("#savempresa #proceso").val(proceso);
}


/////FUNCION PARA ELIMINAR EMPRESA 
function EliminarEmpresa(id, tipo) {
  swal({
    title: "¿Estás seguro?",
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data == 1) {
          ...
          swal("Eliminado!", "Datos eliminados con éxito!", "success");
          $("#restaurantes").load("consultas.php?CargaRestaurantes=si");

        } else if (data == 2) {

          swal("Oops", "Este Restaurante no puede ser Eliminado, tiene registros relacionados!", "error");

        } else {

          swal("Oops", "Usted no tiene Acceso para Eliminar Restaurantes, no eres el Super Administrador del Sistema!", "error");
...
/////////////////////////////////// FUNCIONES DE SALAS //////////////////////////////////////
// FUNCION PARA MOSTRAR QR MESA EN VENTANA MODAL
function VerQR(codmesa) {

  $('#muestraqrmodal').html('<center><i class="fa fa-spin fa-spinner"></i> Cargando información, por favor espere....</center>');

  var dataString = 'BuscaQRModal=si&codmesa=' + codmesa;
  ...
  $.ajax({
    ...
    data: dataString,
    success: function (response) {
      $('#muestraqrmodal').empty();
      $('#muestraqrmodal').append('' + response + '').fadeIn("slow");
